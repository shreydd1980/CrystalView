...
public class SimpleTetraGenerator : MonoBehaviour
{
    ...
    public void GenerateTetragonal()
    {
        ...
        // Generate only corner atoms (simple tetragonal lattice)
        for (int i = 0; i <= nx; i++)
            for (int j = 0; j <= ny; j++)
                for (int k = 0; k <= nz; k++)
                {
    ...
    List<Vector3> GetTetragonalPlaneIntersections(int h, int k, int l)
    {
        ...
        // Define the 8 vertices of the tetragonal parallelepiped (adjusted for flipped Z)
        ...
        // Determine d value based on which face we're creating (adjusted for flipped Z)
        float d = 0;
        ...
        else if (l > 0) d = Vector3.Dot(normal, Vector3.zero);  // For positive Z Miller index, plane is at Z = 0
        else if (l < 0) d = Vector3.Dot(normal, new Vector3(0, 0, -nz * a));  // For negative Z Miller index, plane is at Z = -nz*a
