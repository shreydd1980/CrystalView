...
public class BCTGenerator : MonoBehaviour
{
    public GameObject atomPrefab;
    public GameObject cylinderPrefab;
    public float a = 1f; // Lattice constant for x and z
    public float c = 1.5f; // Lattice constant for y (tetragonal: c ≠ a)
    ...
    public void GenerateBCT()
    {
        // Ensure c is never equal to a (tetragonal, not cubic)
        if (Mathf.Approximately(c, a))
        {
            c = a + 0.5f;
        }
        ...
        HashSet<Vector3> bodyCenterPositions = new HashSet<Vector3>();

        // Corner atoms (simple tetragonal lattice)
        for (int i = 0; i <= nx; i++)
            for (int j = 0; j <= ny; j++)
                for (int k = 0; k <= nz; k++)
                {
                    Vector3 pos = new Vector3(i * a, j * c, -k * a);
                    atomPositions.Add(pos);
                }

        // Body-centered atoms (center of each cell)
        for (int i = 0; i < nx; i++)
            for (int j = 0; j < ny; j++)
                for (int k = 0; k < nz; k++)
                {
                    ...
                    bodyCenterPositions.Add(center);
                }

        // Create atoms (corners and body centers)
        foreach (var pos in atomPositions)
            atoms.Add(CreateAtom(pos));
        foreach (var pos in bodyCenterPositions)
            atoms.Add(CreateAtom(pos));

        // Only connect bonds between corner atoms along cuboid edges (not diagonals, not body centers)
        ...
        // Do NOT connect body center atoms to anything

        // Generate tetragonal planes
        if (showPlanes)
        {
            GenerateTetragonalPlanes();
        }
    }

    #region Tetragonal Planes Generation
    void GenerateTetragonalPlanes()
    {
        // Generate the 6 main tetragonal cell face planes
        GenerateTetragonalPlane(1, 0, 0, "(100)"); // Face perpendicular to x-axis (a-direction)
        GenerateTetragonalPlane(-1, 0, 0, "(1̄00)"); // Opposite face
        GenerateTetragonalPlane(0, 1, 0, "(010)"); // Face perpendicular to y-axis (c-direction)
        GenerateTetragonalPlane(0, -1, 0, "(01̄0)"); // Opposite face
        GenerateTetragonalPlane(0, 0, 1, "(001)"); // Face perpendicular to z-axis (a-direction)
        GenerateTetragonalPlane(0, 0, -1, "(001̄)"); // Opposite face
    }

    void GenerateTetragonalPlane(int h, int k, int l, string planeName)
    {
        // Calculate all possible intersection points with tetragonal cell edges
        List<Vector3> intersectionPoints = GetTetragonalPlaneIntersections(h, k, l);
    ...
    List<Vector3> GetTetragonalPlaneIntersections(int h, int k, int l)
    {
        List<Vector3> points = new List<Vector3>();
        
        // Define the 8 vertices of the tetragonal parallelepiped
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k_idx = 0; k_idx < 2; k_idx++)
                {
                    ...
                    vertices[idx] = new Vector3(i * nx * a, j * ny * c, -k_idx * nz * a);
                }

        // Define the 12 edges of the tetragonal parallelepiped
        int[,] edges = {
            {0,1}, {2,3}, {4,5}, {6,7}, // edges parallel to x-axis (a)
            {0,2}, {1,3}, {4,6}, {5,7}, // edges parallel to y-axis (c)
            {0,4}, {1,5}, {2,6}, {3,7}  // edges parallel to z-axis (a)
        };

        // Calculate plane equation parameters for tetragonal system
        Vector3 normal = new Vector3(h / a, k / c, l / a).normalized;
        ...
        else if (k > 0) d = Vector3.Dot(normal, new Vector3(0, ny * c, 0));
        else if (k < 0) d = Vector3.Dot(normal, Vector3.zero);
        else if (l > 0) d = Vector3.Dot(normal, new Vector3(0, 0, -nz * a));
    ...
    void CreatePlaneMesh(List<Vector3> points, string planeName)
    {
        ...
        GameObject planeObj = new GameObject($"TetragonalPlane_{planeName}");
