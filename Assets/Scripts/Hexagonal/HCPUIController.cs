...
public class HCPUIController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (generator == null)
        {
            Debug.LogError("HCPGenerator reference is missing!");
            return;
        }
    ...
    void UpdateNXFromInput(string value)
    {
        if (int.TryParse(value, out int result))
        {
            ...
            if (inputNX != null) inputNX.text = generator.nx.ToString(); // Update field to show clamped value
            Regenerate();
        }
        else
        {
            if (inputNX != null) inputNX.text = generator.nx.ToString(); // Reset to current value if invalid
        }
    }
    ...
    void UpdateNYFromInput(string value)
    {
        if (int.TryParse(value, out int result))
        {
            ...
            if (inputNY != null) inputNY.text = generator.ny.ToString(); // Update field to show clamped value
            Regenerate();
        }
        else
        {
            if (inputNY != null) inputNY.text = generator.ny.ToString(); // Reset to current value if invalid
        }
    }
    ...
    void UpdateNZFromInput(string value)
    {
        if (int.TryParse(value, out int result))
        {
            ...
            if (inputNZ != null) inputNZ.text = generator.nz.ToString(); // Update field to show clamped value
            Regenerate();
        }
        else
        {
            if (inputNZ != null) inputNZ.text = generator.nz.ToString(); // Reset to current value if invalid
        }
    }
