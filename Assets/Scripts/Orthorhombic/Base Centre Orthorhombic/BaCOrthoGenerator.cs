...
public class BaCOrthoGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        GenerateBaseCenteredOrthorhombic();
    }

    public void GenerateBaseCenteredOrthorhombic()
    {
        ...
        HashSet<Vector3> baseCenterPositions = new HashSet<Vector3>();
        ...
        // Base-centered atoms (center of top and bottom faces on Y, not Z)
        for (int i = 0; i < nx; i++)
            for (int j = 0; j < ny; j++)
                for (int k = 0; k < nz; k++)
                {
                    Vector3 cellOrigin = new Vector3(i * a, j * b, -k * c);
                    // Bottom face center (y = 0)
                    baseCenterPositions.Add(cellOrigin + new Vector3(a / 2f, 0, -c / 2f));
                    // Top face center (y = b)
                    baseCenterPositions.Add(cellOrigin + new Vector3(a / 2f, b, -c / 2f));
                }
        ...
        // Only connect bonds between corner atoms along cuboid edges (not diagonals, not base centers)
