...
public class FCOrthoGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        GenerateFaceCenteredOrthorhombic();
    }

    public void GenerateFaceCenteredOrthorhombic()
    {
        ...
        HashSet<Vector3> faceCenterPositions = new HashSet<Vector3>();

        // Corner atoms (orthorhombic lattice)
        for (int i = 0; i <= nx; i++)
            for (int j = 0; j <= ny; j++)
                for (int k = 0; k <= nz; k++)
                {
        ...
        for (int i = 0; i < nx; i++)
            for (int j = 0; j < ny; j++)
                for (int k = 0; k < nz; k++)
                {
                    Vector3 cellOrigin = new Vector3(i * a, j * b, -k * c);

                    // XY faces (z = 0 and z = c)
                    faceCenterPositions.Add(cellOrigin + new Vector3(a / 2f, b / 2f, 0));
                    faceCenterPositions.Add(cellOrigin + new Vector3(a / 2f, b / 2f, -c));

                    // XZ faces (y = 0 and y = b)
                    faceCenterPositions.Add(cellOrigin + new Vector3(a / 2f, 0, -c / 2f));
                    faceCenterPositions.Add(cellOrigin + new Vector3(a / 2f, b, -c / 2f));

                    // YZ faces (x = 0 and x = a)
                    faceCenterPositions.Add(cellOrigin + new Vector3(0, b / 2f, -c / 2f));
                    faceCenterPositions.Add(cellOrigin + new Vector3(a, b / 2f, -c / 2f));
                }

        // Create atoms (corners and face centers)
        foreach (var pos in atomPositions)
            atoms.Add(CreateAtom(pos));
        foreach (var pos in faceCenterPositions)
            atoms.Add(CreateAtom(pos));

        // Only connect bonds between corner atoms along cuboid edges (not diagonals, not face centers)
        ...
        // Do NOT connect face center atoms to anything
