...
public class SimpleMonoGenerator : MonoBehaviour
{
    ...
    public void GenerateMonoclinic()
    {
        ...
        // Fixed angles
        float alphaRad = Mathf.Deg2Rad * 90f; // alpha = 90°
        float betaRad = Mathf.Deg2Rad * 90f;  // beta = 90°
        ...
        // Generate only corner atoms (monoclinic lattice)
        for (int i = 0; i <= nx; i++)
            for (int j = 0; j <= ny; j++)
                for (int k = 0; k <= nz; k++)
                {
        ...
        // Only connect bonds between corner atoms along cell edges (not diagonals)
